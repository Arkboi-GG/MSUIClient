...
| `SYSTEM_REAL_PORTALS.md` | Summoned Mage portal apertures, per-session server descriptors/readiness leases, isolated destination-scene streaming, live environmental compositing, and delayed worldport ACK ownership | **Environmental-window vertical slice built 2026-08-13.** Six-by-eight-yard sealed apertures reveal only complete fenced frames after destination support and READY. Full-aperture click and client-swept two-sided walk-through both commit through authoritative stock GameObject use. Server-side crossing verification, scene promotion, exit clipping, and remote actors remain open. |
