...
| `SYSTEM_REAL_PORTALS.md` | Summoned Mage portal apertures, per-session server descriptors/readiness leases, isolated destination-scene streaming, live environmental compositing, and delayed worldport ACK ownership | **Environmental-window vertical slice built 2026-08-13; cast-start warm extension built 2026-08-14.** Existing portals warm at 150 yards and retain through 180 while apertures remain 90/120. An accepted own-player portal START can warm from the server's strict six-row catalog, but exact `GAMEOBJECT_CREATED_BY` binding and a matching authoritative descriptor are still required before render/READY. Full-aperture click and client-swept walk-through use stock GameObject authority; server-side crossing verification and remote actors remain open. |
| `SYSTEM_MOUNTS.md` | Riding: `UNIT_FIELD_MOUNTDISPLAYID`, the steed drawn under the rider, the saddle taken from the mount M2's attachment 0, animation 91, the streamed-unit / local-player split, the dev toolkit (§8) and the cart ability kit (§9) | **Written (Draft 1) — built and verified offline 2026-08-13.** Its headline: **1.12 has no vehicle system, and does not need one — the Thousand Needles rocket cars are mounts with the same attachment set as `Creature\Horse`**. **NAMING TRAP: `AttachedItemRenderer.Mount` is a piece of gear on an attachment point, not a steed.** §7 records the rocket cars' authored 3.16-yard origin offset, which is model data and must not be "fixed" in the renderer |
