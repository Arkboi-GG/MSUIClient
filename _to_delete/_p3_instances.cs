...
public sealed partial class GameLoop
{
    ...
    private bool TravelToMapId(int mapId, Vector3 position, float orientation, string why)
    {
        ...
        bool ok = TravelTo(row, wdt, position, orientation);

        // Re-latch against the DESTINATION's triggers. We may well have arrived
        // inside one - that is how a two-way portal is built - and without this
        // the next frame would send us straight back.
        if (ok && _areaTriggers is not null && _controller is not null)
