...
public sealed partial class GameLoop
{
    ...
    private void DrawPortraitLabPanel()
    {
        ...
        ImGui.SameLine();
        ImGui.BeginDisabled();
        ImGui.RadioButton("Specimen (stage 2C)", false);
        ImGui.EndDisabled();

        SyncPortraitLabSubject();
        if (_labShowUnmasked) BakeLabPortraitIfDirty();
    ...
    {
        PortraitSubject wanted = _labSubject == PortraitLabSubject.Player
            ? PortraitSubject.Player : PortraitSubject.Target;
    ...
    private bool BakeLabTarget()
    {
        ...
        bool derive = _labTuning.HeadFraction != PortraitTuning.Default.HeadFraction ||
                      _labTuning.WindowFraction != PortraitTuning.Default.WindowFraction;
        M2PortraitCamera authoredData = default;
        Matrix4x4 transform = default;
        bool authored = _labTuning.ForceSource != PortraitCameraSource.Bounds &&
        ...
        Camera camera = authored
            ? AuthoredPortraitCamera(authoredData, transform)
            : CreatureBoundsPortraitCamera(target, framing, _labTuning, derive);
        bool drawn = false;
        _labPortrait.Bake(() =>
        {
            if (forcedMissing) drawn = true;
            else drawn = _creatures.RenderPortrait(camera, target);
        });
        PortraitRenderTarget.ReadbackStats stats = _labPortrait.Analyze();
        if (drawn && !stats.HasSubject &&
            _labTuning.ForceSource != PortraitCameraSource.Authored)
        {
            Camera fallback = CreatureBoundsPortraitCamera(target, framing, _labTuning, derive);
            _labPortrait.Bake(() => drawn = _creatures.RenderPortrait(fallback, target));
            stats = _labPortrait.Analyze();
        }
        return drawn && stats.HasSubject;
    }
}
