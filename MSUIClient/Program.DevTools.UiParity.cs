...
public sealed partial class GameLoop
{
    ...
    private void ArmUiParityCapture(string panel)
    {
        if (!_config.DevTools || panel is not ("game-menu" or "player-frame" or "target-frame" or "party-frame" or
            "action-bar" or "action-button" or "multi-action-bar" or "cast-bar" or "buff-frame" or "minimap" or "chat-frame" or "reputation-bar" or "backpack" or "character-frame" or "spellbook" or "talent-frame" or "quest-log" or "merchant" or "trainer" or "bank" or "mail" or "auction")) return;
        ...
        string parent, UiParityDrawTrace trace)
    {
        ...
        static string Norm(string path) => path.Length == 0 || path.EndsWith(".blp", StringComparison.OrdinalIgnoreCase) ? path : path + ".blp";
        float logicalScale = MathF.Max(_uiParityLogicalScale, 0.001f);
        Vector2 relative = element is "PlayerFrame" or "TargetFrame" or "CharacterFrame" or "PaperDollFrame" or "SpellBookFrame" or "TalentFrame" or "QuestLogFrame" or "MerchantFrame" or "ClassTrainerFrame" or "BankFrame" or "MailFrame" or "AuctionFrame" ? Vector2.Zero : (min - _uiParityOrigin) / logicalScale;
