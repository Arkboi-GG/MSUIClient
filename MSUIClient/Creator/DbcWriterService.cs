...
///   Records: recordCount Ã— recordSize bytes (flat uint32 array; floats and stringrefs are also 4 bytes)
///   StringBlock: null-terminated strings referenced by uint32 offsets in record fields
/// </summary>
public class DbcWriterService
{
    // â”€â”€ Per-file state â”€â”€
    ...
    private Dictionary<uint, string> _strings = new(); // stringBlock offset â†’ string value
    private byte[] _originalStringBlock = Array.Empty<byte>();

    // â”€â”€ Indexed lookup: field[0] (usually ID) â†’ row index â”€â”€
    private Dictionary<uint, int> _idIndex = new();

    // â”€â”€ Read from file â”€â”€
    ...
    public static DbcWriterService ReadDbc(byte[] data, string sourcePath = "")
    {
        ...
        // Parse records â€” each field is 4 bytes (uint32/int32/float all fit in 4 bytes)
    ...
    /// resolves a record by BINARY SEARCH on that assumption â€” it does not scan.
    ...
    /// walks custom_item_retexture first, then custom_item_retexture_atlas â€” and
    ...
    ///     resulting tail:       60001, 60005, 60009, 60002, 60003, 60004  â† NOT SORTED
    ///
    ...
    /// only the custom displays resolve to someone else's row â€” a glove wearing a
    ...
    public byte[] Write()
    {
        // MUST come before serialization â€” see SortById. An unsorted DBC does not
