...
///   0x0D0-0x130: Empty M2Arrays (lights, cameras, ribbons â€” all zero for spell effects)
///   0x134:       M2Array â€” secondary emitters (ribbon-like, simpler struct ~0xDC bytes)
///   0x13C:       M2Array â€” particle emitters (full struct, 0x1F8 = 504 bytes each) â† PRIMARY
///
/// Particle emitter struct (M2Particle, 0x1F8 = 504 bytes):
///
/// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
/// VANILLA v256 M2Track<float> FORMAT â€” EMPIRICALLY VERIFIED (Session 11)
/// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
///
/// Each M2Track<float> in vanilla is 28 bytes, starting with 0x0000FFFF marker:
///   +0x00: 0x0000FFFF (4 bytes) â€” separator/marker
///   +0x04: 0x00000000 (4 bytes) â€” padding
///   +0x08: 0x00000000 (4 bytes) â€” padding
///   +0x0C: M2Array count (uint32) â€” timestamp count (matches value count)
///   +0x10: M2Array offset (uint32) â€” abs file offset to timestamp data
///   +0x14: M2Array count (uint32) â€” value count (usually 1 for static)
///   +0x18: M2Array offset (uint32) â€” abs file offset to float value(s)
///
...
///     +0x150: uint32[3] colorValues START/MID/END (ARGB) â† PATCHED (existing)
///     +0x15C: float[3]  scaleValues START/MID/END â† PATCHED (existing)
...
///   0      +0x034   +0x048    emissionSpeed          0.17â€“6.46 m/s
///   1      +0x050   +0x064    speedVariation         0.07â€“0.72 (fraction)
///   2      +0x06C   +0x080    verticalRange          0.017â€“Ï€ radians
///   3      +0x088   +0x09C    horizontalRange        0â€“2Ï€ radians
///   4      +0x0A4   +0x0B8    gravity                -3.47 to 6.67
///   5      +0x0C0   +0x0D4    lifespan               0.25â€“1.0 seconds
///   6      +0x0DC   +0x0F0    emissionRate           10â€“74 particles/sec â˜…
///   7      +0x0F8   +0x10C    emissionAreaLength     0.006â€“0.83 units
///   8      +0x114   +0x128    emissionAreaWidth      0.006â€“0.83 units
///   9      +0x130   +0x144    zSource/deceleration   rarely non-zero
///
/// âš ï¸ SESSION 9: Removed PatchM2TracksInRange â€” it was corrupting bone IDs.
/// âœ… SESSION 11: Replaced with proper M2Track parser that follows M2Array
...
public class M2ParticlePatcher
{
    ...
    public class ParticlePatchParams
    {
        ...
        /// When UseHueShift is true, these are ignored â€” HueShiftColor is used instead.</summary>
    ...
    public static byte[]? PatchParticles(byte[] m2Data, ParticlePatchParams patchParams)
    {
        ...
        // â”€â”€ Patch primary emitters (0x13C) â€” the main particle effects â”€â”€
        ...
        // â”€â”€ Patch secondary emitters (0x134) â€” simpler ribbon-like effects â”€â”€
    ...
    {
        // â”€â”€ Inline fixed values (existing â€” verified Session 10) â”€â”€

        // Color values at +0x150 (3 Ã— uint32 ARGB)
        if (p.UseHueShift)
        {
        ...
        // Scale values at +0x15C (3 Ã— float)
        ...
        // â”€â”€ M2Track animated values (NEW â€” Session 11) â”€â”€
        // Follow M2Array pointers to patch the actual float data in the file.
        // No structural changes â€” just modify the float(s) at the target offset.
    ...
    ///   [0:4] uint32 count â€” number of float values
    ///   [4:8] uint32 offset â€” absolute file offset to the float array
    /// 
    ...
    /// This makes multi-emitter M2s look natural â€” a bright core stays bright,
