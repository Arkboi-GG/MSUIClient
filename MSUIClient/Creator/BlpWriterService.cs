...
/// <summary>
/// Converts PNG â†’ BLP2 for vanilla WoW 1.12.1 particle textures and icons.
...
///             Non-square BLP support (e.g. Ribbon 64Ã—32).
/// Session 27: POST-RESIZE PROCESSING
///             All post-processing (lum-alpha, vignette, gradient clamping)
///             now happens at FINAL vanilla resolution, not at FLUX 512Ã—512.
...
/// Session 28: BRIGHTNESS FLOOR MASK â€” Content-Aware Alpha Carving
...
public class BlpWriterService
{
    ...
    // PUBLIC API â€” PNG â†’ BLP CONVERSION (LEGACY â€” resize inside)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    /// <summary>
    /// Convert a PNG file to BLP2 DXT3 bytes (alphaDepth=8).
    /// Square output â€” targetSize applied to both width and height.
    ...
    /// <summary>
    /// Internal conversion â€” handles all combinations of format and dimensions.
        ...
        bool useDxt1, bool applyLumAlpha)
    {
        try
        {
            ...
            if (resized == null)
            {
                _logger?.LogError("BlpWriter: Resize to {W}Ã—{H} failed", targetWidth, targetHeight);
                return null;
            }
    ...
    //   1. ResizePngToBitmap (512â†’32, get pixel array)
    //   2. ApplyLuminanceAlphaPixels (on 32Ã—32 pixels)
    //   3. ApplyRadialVignettePixels (on 32Ã—32 pixels)
    //   4. EncodeBitmapToBlp (DXT encode, no resize)
    //
    // Everything operates at final resolution. No more vignetting
    // at 512Ã—512 then crushing to 32Ã—32.
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    public SKBitmap? ResizePngToBitmap(string pngPath, int targetWidth, int targetHeight)
    {
        try
        {
            ...
            _logger?.LogDebug("BlpWriter: Resized {Path} ({SrcW}Ã—{SrcH}) â†’ {TgtW}Ã—{TgtH}",
    ...
    /// Operates on the actual bitmap pixels â€” no file I/O.
    /// </summary>
    ...
    /// For DXT3 + additive: renderer does dest += src.RGB Ã— src.Alpha,
    ...
    // content â€” irregular, organic, blobby â€” exactly like a hand-painted
    // vanilla texture.
    //
    // Why this works:
    ...
    //   (near-black pixels at corners/edges) is NOT the particle â€” but at
    ...
    //
    // For DXT1 (no alpha): zeroing RGB is sufficient â€” additive does dest += RGB.
    // For DXT3 (has alpha): zeroing BOTH RGB and alpha â€” additive does
    //   dest += RGB Ã— Alpha, so both must be zero.
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    /// additive (dest += RGB Ã— Alpha) blending modes.
    /// </summary>
    ...
    public void ApplyBrightnessFloorMask(SKBitmap bitmap, float floorPercent = 0.12f, float kneeWidth = 0.08f)
    {
        ...
        // â”€â”€ Step 1: Find peak brightness across all pixels â”€â”€
        float peakLum = 0f;
        for (int i = 0; i < pixels.Length; i++)
        {
            var c = pixels[i];
        ...
        if (peakLum < 1f)
        {
            _logger?.LogDebug("BlpWriter: BrightnessFloor â€” image is pure black, skipping");
            return;
        }
        ...
        // â”€â”€ Step 2: Kill below floor, smooth-fade in knee band â”€â”€
        for (int i = 0; i < pixels.Length; i++)
        {
            var c = pixels[i];
            float lum = 0.299f * c.Red + 0.587f * c.Green + 0.114f * c.Blue;
            ...
            if (lum <= floorLum)
            {
                // Below floor: dead pixel â€” zero everything
            ...
            // else: above knee â€” untouched, full brightness preserved
        }
        ...
        _logger?.LogInformation(
            "BlpWriter: BrightnessFloor applied â€” peak={Peak:F0} floor={Floor:F1} knee={Knee:F1} " +
        ...
        float floorPercent = 0.12f, float kneeWidth = 0.08f)
    {
        ...
        bitmap.Pixels = pixels;
        _logger?.LogInformation(
            "BlpWriter: AtlasBrightnessFloor applied â€” {Grid}x{Grid} cells, killed={Killed}/{Total}",
            gridSize, gridSize, totalKilled, pixels.Length);
    }

    /// <summary>
    /// Session 27: Encode an already-processed SKBitmap directly to BLP2 bytes.
    /// No resize, no luminance-to-alpha â€” the bitmap must already be at final
    ...
    public byte[]? EncodeBitmapToBlp(SKBitmap bitmap, bool useDxt1)
    {
        try
        {
            ...
            {
                _logger?.LogError("BlpWriter: EncodeBitmapToBlp requires power-of-2 dims â‰¥4, got {W}Ã—{H}", w, h);
                return null;
            }

            string fmt = useDxt1 ? "DXT1" : "DXT3";
            _logger?.LogInformation("BlpWriter: EncodeBitmapToBlp {W}Ã—{H} {Fmt}", w, h, fmt);
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // BLP2 DXT3 ENCODER (existing â€” alphaDepth=8, alphaType=1)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    private byte[] EncodeBlpDxt3(SKBitmap image)
    {
        ...
        try
        {
            ...
            _logger?.LogInformation("BlpWriter: DXT3 â€” {Count} mipmap levels", mipmaps.Count);
            ...
            _logger?.LogInformation(
                "BlpWriter: Encoded BLP2 DXT3 â€” {W}Ã—{H}, {Mips} mips, {Bytes} bytes",
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // BLP2 PALETTIZED ENCODER â€” compression=1, alphaDepth=8
    //
    ...
    // indexed pixels â€” vanilla ships every one of them palettized. Hand it a DXT
    ...
    // (not DXT1) â€” vanilla stores them as plain BLP". The intent was right; the
    // encoder to honour it never existed. useDxt1:false means DXT3, not
    // uncompressed. Every component texture ever committed went out as DXT3 â€”
    ...
    // holds <=256 unique colours and the palette is built EXACTLY â€” lossless. The
    ...
    /// alphaDepth=8) â€” the format vanilla uses for character component textures.
    /// </summary>
    ...
    private byte[] EncodeBlpPalettized(SKBitmap image)
    {
        ...
        try
        {
            ...
            _logger?.LogInformation(
                "BlpWriter: palettized â€” {N} palette entries ({Mode}), {Mips} mips",
            ...
            bw.Write(new byte[] { (byte)'B', (byte)'L', (byte)'P', (byte)'2' });
            bw.Write((uint)1);    // type = 1 (direct content)
            bw.Write((byte)1);    // compression = 1 (UNCOMPRESSED, palettized)  â† the fix
            ...
            var result = ms.ToArray();
            _logger?.LogInformation(
                "BlpWriter: Encoded BLP2 PALETTIZED â€” {W}Ã—{H}, {Mips} mips, {Bytes} bytes",
                width, height, encodedMips.Count, result.Length);
            return result;
        }
        finally
        ...
        SKBitmap mip, List<uint> palette, Dictionary<uint, byte> exactLookup)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                ...
                buf[n + i] = px.Alpha;        // 8-bit alpha plane â€” the belts depend on this
            }
        }
        return buf;
    }
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // BLP2 DXT1 ENCODER (Session 24 â€” alphaDepth=0, alphaType=0)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    private byte[] EncodeBlpDxt1(SKBitmap image)
    {
        ...
        {
            mipmaps.AddRange(BuildMipmapChain(image));
            _logger?.LogInformation("BlpWriter: DXT1 â€” {Count} mipmap levels", mipmaps.Count);
            ...
            var result = ms.ToArray();
            _logger?.LogInformation(
                "BlpWriter: Encoded BLP2 DXT1 â€” {W}Ã—{H}, {Mips} mips, {Bytes} bytes",
                width, height, encodedMips.Count, result.Length);
            return result;
        }
        finally
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // MIPMAP CHAIN (supports non-square â€” stops when BOTH w and h reach 1)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    {
        // Alpha block (8 bytes): 16 Ã— 4-bit alpha values
        for (int i = 0; i < 8; i++)
        {
            int a0 = a[i * 2] >> 4;
            int a1 = a[i * 2 + 1] >> 4;
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // SHARED COLOR BLOCK ENCODER (8 bytes â€” used by both DXT1 and DXT3)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    /// <summary>
    /// Encode 16 pixels as an 8-byte DXT color block (2 RGB565 endpoints + 16Ã—2-bit indices).
    /// Uses bounding-box endpoint selection with 4-color palette (c0 >= c1 â†’ 4 colors).
    /// </summary>
    ...
    /// <summary>
    /// Apply a radial vignette to ALPHA ONLY â€” RGB stays untouched.
    /// For blendMode=2 (alpha blend) emitters.
    ...
    /// <summary>
    /// Apply a per-cell radial vignette to a sprite atlas â€” ALPHA ONLY.
    ...
    /// <summary>
    /// Apply a radial vignette to RGB channels â€” fades toward black at edges.
    ...
    /// <summary>
    /// Apply a per-cell radial vignette to a sprite atlas â€” RGB channels only.
    ...
    public void ApplyLuminanceAlpha(string pngPath)
    {
        try
        {
            ...
            if (bitmap == null)
            {
                _logger?.LogWarning("BlpWriter: ApplyLuminanceAlpha â€” could not decode {Path}", pngPath);
                return;
            }
            ...
            _logger?.LogDebug("BlpWriter: Applied luminanceâ†’alpha to {Path} ({W}x{H})",
