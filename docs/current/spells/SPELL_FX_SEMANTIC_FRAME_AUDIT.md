...
| `tools/spell-particle-motion-check` output: `PASS (80 checks)` | `STATIC/DATA_COMPLETE` | The full 9,717-path mounted census, 2,550 special records, reference fixtures, Blizzard frame/asset facts, pivot-rebased full-TRS round-trip, hitch cadence, and the mandatory two-sided/opposite-winding tail law agree with production. | Recorded matched original-client runtime pixels. |
