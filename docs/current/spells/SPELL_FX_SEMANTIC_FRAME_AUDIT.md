...
implementation and a passing numeric regression check. No matched original-client/MSUI Blizzard capture has
been made after that correction, so the Blizzard wake is **not** `CAPTURED` or `PARITY_CERTIFIED`.
...
| `R(t)` | Effect/model instance root-to-world placement. Translation is the ordinary particle cloud anchor. | `SpellEffectSource.TryTransform` | Mesh, particles, ribbons, decals. It is not an emitter bone and not the lifetime owner. |
...
| `C(t)` | Root cloud anchor position: translation of `R(t)`. | Particle instance input | Draw anchor for ordinary non-model particles. Never substitute current emitter position. |
...
| `S_anchor` | Persistent ordinary-particle position: attach-local offset from `C(t0)`. | `SpellParticleFrameLaw.StoreAtBirth` | Draw is `C(t) + A(t) * S_anchor`; no `B_i(t)` term. |
...
| `S_world` | Absolute world storage used by a lane explicitly proven world-history-based. | Lane-specific birth | Current MSUI missile behavior remains here pending matched evidence. |
...
|---|---|---|---|---|
| Ordinary anchored particle | `birthWorld(t0) = R(t0) * B_i(t0) * K` | `S_anchor = A(t0)^-1 * (birthWorld(t0) - C(t0))` | `W(t) = C(t) + A(t) * S_anchor`; explicitly no `B_i(t)` | Static implementation + numeric check; live asset open. |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| P-046 | Particle | Ordinary live particles anchor translation at the effect/model root, not the animated emitter bone. | M2 flag `0x10` clear; effect instance root; evaluated emitter bone | Root cloud anchor | `birthWorld` in `W(t0)` | `S_anchor` | Birth; root sampled again at draw | Benilla `particles.rs:130-142`, `entities/spell_fx.rs:448-464`, `particles/sim.rs:601-619,728-746`; original capture still required | `SpellParticleSystem.cs:110-127,202-220,682-716,951-968`; pure operations in `SpellParticleFrameLaw.cs` | `Spells\Blizzard_Impact_Base.m2` and a second animated-bone emitter | With stationary root and moving bone, old zero-velocity particles remain at their birth positions and cloud span grows along bone history | static-match after correction | `STATIC_FOUND` | Wrong law collapses Blizzard's wake and drags orbiting clouds. |
...
| P-048 | Particle | Root motion carries ordinary non-model clouds. | Effect root translation | Root | `S_anchor` | `W(t)` | Draw | Benilla `particles.rs:63-77,130-142`; `sim.rs:731-746` | `RootCarriesCloud` is separate from emitter and attachment rotation | Moving owner/static bone fixture | Root translation `d` moves every old particle by exactly `d` | static-match | `STATIC_FOUND` | A false world-freeze leaves hand/body clouds behind. |
...
| O-024 | Missile orchestration/frame | Release, destination lookup, fixed deadline, homing, pose, cloud ownership, sound, and impact are one pipeline. | Cast-kit animation; M2 release events; SpellVisual missile fields; Spell speed; live caster/target poses | Pending GO, then missile root, then impact owner | Release marker in caster model space; destination/root in `W`; parsed missile model in Y-up `F_model` | Root-relative ordinary particle state with identity free-model attachment rotation; ribbon nodes retain their own committed history | GO fixes deadline; release edge launches; every flight tick re-resolves target and consumes raw dt; impact before snap | Benilla `entities/missile.rs`, `entities/spell_fx.rs:280-310,448-470`, `particles.rs:119-142`, `particles/sim.rs:402-451,601-619,728-746` | `SpellMissileLaw`; `SpellEffectSource`; explicit no-tag sentinel in `SpellVisualCatalog`; no particle-step clamp in mover | 981 speed spells, 64 distinct missile paths, Arcane Shot follow model, synthetic marker/finish/backstop/no-tag/expired-deadline fixtures | +X faces flight; parsed +Y remains up; no roll; absent/out-of-range destination begins at `0x0F/0x13`; old ordinary particles move only with live root translation; close release may impact without creating flight/audio | static/data complete; original capture open | `STATIC_FOUND` | Independent approximations caused wrong launch time/point, chest-biased basic shots, 100 ms hitch lag, rotated flight axes, detached clouds, or a one-frame impact snap. |
...
| Head/tail quad | Archive census selection with actual head/tail record | Quad expansion | Static branch | Asset dump plus capture. |
...
Runtime-certified by this pass: **nothing new**.  
...
409 of the 555 resolved referenced models. The validator currently passes 5,788 checks, including 579 exact
