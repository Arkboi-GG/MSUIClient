...
static void CheckCompressedObjectUpdate()
{
    ...
    using (var compressedWriter = new BinaryWriter(
               compressedStream, System.Text.Encoding.UTF8, leaveOpen: true))
    {
        compressedWriter.Write(updateBody.Length);
        using var zlib = new ZLibStream(
            compressedStream, CompressionLevel.Fastest, leaveOpen: true);
...
if (args.Contains("--compressed-update-only", StringComparer.OrdinalIgnoreCase))
{
    CheckCompressedObjectUpdate();
    Console.WriteLine("compressed object-update checks passed");
    return;
}
