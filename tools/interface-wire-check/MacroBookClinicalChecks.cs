...
internal static class MacroBookClinicalChecks
{
    ...
    private static void CheckRuntimeSourceFence()
    {
        ...
        string bars = File.ReadAllText(Path.Combine(client, "GameLoop", "Hud",
            "GameLoop.ActionBars.cs"));
        string store = File.ReadAllText(Path.Combine(client, "Engine", "MacroStore.cs"));
        ...
        Check(macro.Contains("UiPanelFrameLogicalOrigin(UiPanelOwnershipRegistry[15])", StringComparison.Ordinal) &&
              ...
              macro.Contains("MacroStore.Paths(", StringComparison.Ordinal) &&
              macro.Contains("MacroStore.MigrateLegacy(", StringComparison.Ordinal) &&
              store.Contains("Path.Combine(root, \"macros\", \"accounts\", accountKey)", StringComparison.Ordinal) &&
